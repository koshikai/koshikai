...
`src/app/share/hvac-precooling-9d4c7b2e6a13/`（空調の最適起動分析コード・スライド共有ページ）は、
インターン実習・成果発表会の終了に伴い、機密保持と運用終了のため非公開化（削除）済み。

再度公開が必要になった場合は、`koshikai/panasonic-ew` リポジトリでエクスポートスクリプトを実行して反映する。
